...
RSpec.describe "Company Settings" do
  ...
  context "when team updates flag is enabled" do
    let(:client_id) { GlobalConfig.get("GH_CLIENT_ID") }
    let(:client_secret) { GlobalConfig.get("GH_CLIENT_SECRET") }
    let(:code) { "test_code" }
    let(:state) { Base64.strict_encode64("#{company.external_id}:#{company.name}") }
    let(:access_token) { "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" }
    let(:authentication_token) { Base64.strict_encode64("#{client_id}:#{client_secret}") }

    before { company.update!(team_updates_enabled: true) }

    before do
      proxy.stub("https://github.com:443/login/oauth/authorize")
           .and_return(redirect_to: oauth_redirect_url(code:, state:))
      ...
      WebMock.stub_request(:get, "https://api.github.com/user")
              .to_return(
                status: 200,
                body: { id: 1234567890, login: "gumroad" }.to_json,
                headers: { content_type: "application/json" },
              )

      WebMock.stub_request(:get, "https://api.github.com/user/memberships/orgs?state=active")
              .to_return(
                status: 200,
                body: [{ organization: { login: "antiwork" } }].to_json,
                headers: { content_type: "application/json" },
              )

      WebMock.stub_request(:post, "https://api.github.com/orgs/antiwork/hooks")
              .to_return(status: 200, body: { id: "1234567890" }.to_json, headers: { content_type: "application/json" })

      WebMock.stub_request(:delete, "https://api.github.com/applications/#{client_id}/grant")
              .to_return(status: 204)
    end

    it "allows connecting and disconnecting GitHub", :billy do
      sign_in admin_user
      visit spa_company_administrator_settings_path(company.external_id)
      expect(page).to have_selector("h1", text: "Company account")
      expect(page).to have_selector("h2", text: "GitHub")
      expect(page).to have_text("Unfurl GitHub pull request and issue URLs within your team updates.")
      ...
      expect do
        ...
        expect(page).to have_text("Connected")
        expect(page).to have_button("Disconnect")
      end.to change { GithubIntegration.count }.by(1)

      integration = GithubIntegration.last
      expect(integration.status).to eq(Integration.statuses[:active])
      expect(integration.access_token).to eq(access_token)
      expect(integration.account_id).to eq("1234567890")
      expect(integration.organizations).to eq(["antiwork"])

      expect do
        click_on "Disconnect"
        wait_for_ajax
        expect(page).to_not have_text("Connected")
        expect(page).to have_button("Connect")
      end.to change { integration.reload.status }.from(Integration.statuses[:active]).to(Integration.statuses[:deleted])
        .and change { GithubIntegration.count }.by(0)
    end
  end
