...
RSpec.describe Stripe::EventHandler do
  ...
  describe "issuing transaction events" do
    context "when event type is issuing_transaction.created" do
      let(:expense_card) { create(:expense_card) }
      let(:stripe_event) do
        Stripe::Event.construct_from(
      ...
      it "creates a new ExpenseCardCharge" do
        expect do
          described_class.new(stripe_event).process!
        end.to change(ExpenseCardCharge, :count).by(1)
      end

      it "sets the correct attributes on the created ExpenseCardCharge" do
        described_class.new(stripe_event).process!
        charge = ExpenseCardCharge.last

        expect(charge.description).to eq("Merchant Name, Merchant City, US")
        expect(charge.expense_card).to eq(expense_card)
        expect(charge.company).to eq(expense_card.company)
        expect(charge.total_amount_in_cents).to eq(50_00)
        expect(charge.processor_transaction_reference).to eq("ipi_1234567890")
        expect(charge.processor_transaction_data).to eq(stripe_event.data.object.as_json)
      end

      it "raises an exception if a matching expense card isn't found" do
    ...
    context "when event type is issuing_transaction.updated" do
      let!(:expense_card_charge) { create(:expense_card_charge, processor_transaction_reference: "ipi_1234567890") }
      let(:stripe_event) do
        Stripe::Event.construct_from(
      ...
      it "updates the existing ExpenseCardCharge" do
        expect do
          described_class.new(stripe_event).process!
        end.to change { expense_card_charge.reload.processor_transaction_data }
      end

      it "sets the correct updated attributes on the ExpenseCardCharge" do
        described_class.new(stripe_event).process!
        expense_card_charge.reload

        expect(expense_card_charge.processor_transaction_data).to eq(stripe_event.data.object.as_json)
      end

      it "raises an exception if a matching expense card charge isn't found" do
        expense_card_charge.destroy!
